...
foreach ($input as $nombre => $valor) { //Se recorre todos los objetos que se han mandado desde el controlador de angularjs
	if($nombre == "tabla"){ //si el valor que se mando se llama query, pasara a una variable unica
		$query = "UPDATE ".mysqli_real_escape_string($conn, $valor)." SET ";
	}else if($nombre == "where"){ //Si el valor que se mando se llama id, pasara a una variable unica concatenado con su WHERE
		$where = "WHERE ".$valor;
	}else if($nombre == "ID"){
